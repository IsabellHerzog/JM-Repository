...
function moveBalken(relativeMousePosition) {
    ...
    balken.style.transform = `translate(-50%, ${-200 * relativeMousePosition.y}%)`;
}

function moveTextBlocks(relativeMousePosition) {
    const textBlocks = document.getElementsByClassName('text');
    textBlocks[0].style.transform = `translateX(${-50 * relativeMousePosition.y}%)`;
    textBlocks[1].style.transform = `translateX(${50 * relativeMousePosition.y}%)`;
}
