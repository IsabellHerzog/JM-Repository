...
var contentAbove = 0;
var gardenSize = 9000;
var pillarSize = 300; //dimensions of the rects
const scaleW = 1680; //width for which was designed for
var fuzzyRadius = 40; //spreading of the light
var backgroundColor = "rgba(1, 0, 0, 0.3)";
var shadowSideStripes = "rgba(100,100,100,0.1)";
...
function draw(){
	...
	for(var i=0;i<rectPlaces.length;i++){
		...
		ctx.lineTo(spot.x+pillarSize,spot.y -windowOffset);
		ctx.lineTo(spot.x+pillarSize,spot.y + pillarSize -windowOffset);
		ctx.lineTo(spot.x,spot.y+pillarSize -windowOffset);
		ctx.fill();
	}
}
...
calcSegments(0,1, gardenSize)
calcSegments(1, rectPlaces.length-1, pillarSize);
// calcSegments();
...
window.onload = function(){
	...
	lightSpotImage.src = "assets/garden_lightShaftSpot.png";
};
